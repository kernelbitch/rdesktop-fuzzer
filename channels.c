...
void
channel_send_hooked(STREAM s, VCHANNEL * channel)
{
	uint32 length, flags;
...
#endif
	DEBUG(("Caught hooked call channel_send_hooked()\n"));
...
{
	DEBUG(("Caught call for hooked channel_send()\n"));
	channel_send_hooked(s, channel);
}
