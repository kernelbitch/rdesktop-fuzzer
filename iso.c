...
static void
iso_send_msg_hooked(uint8 code)
{
	STREAM s;
	DEBUG(("Caught hooked call to iso_send_msg();\n"));
	s = tcp_init(11);
...
{
	iso_send_msg_hooked(code);
}

static void
iso_send_connection_request_hooked(char *username)
{
	STREAM s;
	DEBUG(("Caught hooked call to iso_send_connection_request();\n"));
...
{
	iso_send_connection_request_hooked(username);
}
...
void
iso_send_hooked(STREAM s)
{
	uint16 length;

	DEBUG(("Caught hooked call to iso_send()\n"));
