...
void
mcs_send_to_channel_hooked(STREAM s, uint16 channel)
{
	uint16 length;
	DEBUG(("Caught hooked call to mcs_send_to_channel()\n"));
...
{
	mcs_send_to_channel_hooked(s, channel);
}

/* Send an MCS transport data packet to the global channel */
void
mcs_send_hooked(STREAM s)
{
	DEBUG(("Caught call for hooked mcs_send()\n"));
